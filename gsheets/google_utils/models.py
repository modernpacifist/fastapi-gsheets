...
class GetConcreteConference(Schema):
    ...
    id = fields.Int()
    name_rus = fields.Str()
    name_rus_short = fields.Str()
    name_eng = fields.Str()
    name_eng_short = fields.Str()
    registration_start_date = fields.Date(format="%d.%m.%Y")
    registration_end_date = fields.Date(format="%d.%m.%Y")
    submission_start_date = fields.Date(format="%d.%m.%Y")
    submission_end_date = fields.Date(format="%d.%m.%Y")
    conf_start_date = fields.Date(format="%d.%m.%Y")
    conf_end_date = fields.Date(format="%d.%m.%Y")
    organized_by = fields.Str()
    url = fields.URL()
    email = fields.Email()
